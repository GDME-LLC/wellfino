        ...
        <div className="flex items-center gap-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground shadow-sm transition-all hover:brightness-110 hover:shadow-md"
      ...
      {mobileOpen && (
        <div className="border-t border-border/40 bg-background px-6 py-4 md:hidden">
          ...
          <Link
            ...
            onClick={() => setMobileOpen(false)}
            className={`block py-3 text-sm font-medium transition-colors hover:text-primary ${
              location.pathname === "/best-sellers" ? "text-primary" : "text-muted-foreground"
            }`}
          >
            Best Sellers
          </Link>
